...
public class ReviewController {
    ...
    public ResponseEntity addReviews(){
        Review review1 = new Review(1,10,1,4,"Good stuff");
        Review review2 = new Review(2,10,1,5,"Good stuff");
        Review review3 = new Review(3,10,1,2,"Broken");
        Review review4 = new Review(4,10,3,4,"Good stuff");
    ...
    public ResponseEntity updateReview(@PathVariable int id, @Valid @RequestBody Review review, Errors errors){
        if (errors.hasErrors()) {
            String message = errors.getFieldError().getDefaultMessage();
            return ResponseEntity.status(400).body(new ApiResponse(message));
        }
